...
static void* check_bus_state(void* ptr)
{
    ...
    while (running)
    {
        ...
        for (int j = 1; j <= groupcount; j += 1)
        {
            ...
            if (!state_read)
            {
                log_info("Error on Group %d, reading State.", j);
            ...
            for (int i = 1; i <= ec_slavecount; i += 1)
            {
                ...
                {
                    log_info("Slave %d not in operational state (%02x), current state (%02x)",
                             i,
                ...
                if (ec_slave[i].state == 0x00)
                {
                    ...
                    log_info("Slave %d is requesting state (%02x).", i,
                             ec_slave[i].state);
                ...
                else if (ec_slave[i].state == EC_STATE_PRE_OP)
                {
                    ...
                    log_info("Applying Slave configuration for slave %d.", i);
                    int ret = ec_config_apply(i);

                    if (ret == -1)
                    {
                        log_error("Error applying settins for slave %d.", i);
                    }
                ...
                else if (ec_slave[i].state == EC_STATE_SAFE_OP)
                {
                    ...
                    if (ret == -1)
                    {
                        log_error("Error applying config for slave %d.", i);
                    }
                ...
                {
                    log_error("Slave %d in unknown state (%02x).", i, ec_slave[i].state);
                }
            }
...
static void ec_on_init(master_t* master)
{
    ...
    int retConfAppAll = ec_config_apply_all();

    if (retConfAppAll == -1)
    {
        log_error("Error applying slave configuration.");
    }
